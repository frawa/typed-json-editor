...
export class TypedJsonConnectLocal implements TypedJsonConnect {
  ...
  private localValidate: LocalValidateFun = window[
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    'validate'
  ] as LocalValidateFun;
  private localValidateSchema: LocalValidateSchemaFun = window[
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    'validateSchema'
  ] as LocalValidateSchemaFun;
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
  private localSuggest: LocalSuggestFun = window['suggest'] as LocalSuggestFun;
  private localSuggestSchema: LocalSuggestSchemaFun = window[
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    'suggestSchema'
  ] as LocalSuggestSchemaFun;
  ...
  private doSuggest(
    ...
    try {
      ...
      const o: readonly SuggestionOutput[] = parseSuggestionOutput(
        JSON.parse(result)
      );
