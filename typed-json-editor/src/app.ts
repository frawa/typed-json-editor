...
// WTF?
(
  editorJson.getContribution("editor.contrib.suggestController") as any
).widget.value._setDetailsVisible(true);
(
  editorSchema.getContribution("editor.contrib.suggestController") as any
).widget.value._setDetailsVisible(true);
...
languages.registerCompletionItemProvider("json", {
  ...
  provideCompletionItems: (model, position, context) => {
    const result = null; // typedJsonById[model.id]().suggestionsAt(model.getOffsetAt(position))
    if (!result) {
      return {
        suggestions: [],
      };
...
document
  .querySelector<HTMLSelectElement>("#sample-schema")
